...
public class MainActivity extends Activity {
    ...
    void buildNetease(){base("网易云下载");
        TextView sub=text("网易云音乐下载",13);sub.setTextColor(muted());content.addView(sub,new LinearLayout.LayoutParams(-1,dp(30)));
    ...
    void startUrlDownload(String url,String filename){
        ...
        Executors.newSingleThreadExecutor().execute(()->{
            ...
            try{
                ...
                c.setRequestProperty("Accept","audio/mpeg,audio/*,*/*;q=0.8");

                int code=c.getResponseCode();
                if(code<200 || code>=300) throw new IOException("网易云返回 HTTP "+code);

                String contentType=c.getContentType();
                long len=c.getContentLengthLong();

                // The outer URL should redirect to an actual audio resource.
                // Refuse obvious HTML/error pages instead of saving them as .mp3.
                if(contentType!=null){
                    String ct=contentType.toLowerCase(Locale.US);
                    if(ct.contains("text/html") || ct.contains("application/json") || ct.contains("text/plain")){
                        throw new IOException("网易云没有返回 MP3 音频（Content-Type: "+contentType+"）");
                    }
                }
                ...
                if(Build.VERSION.SDK_INT>=29){
                    ...
                    v.put(MediaStore.Downloads.MIME_TYPE,"audio/mpeg");
                    v.put(MediaStore.Downloads.RELATIVE_PATH,Environment.DIRECTORY_DOWNLOADS);
                    v.put(MediaStore.Downloads.IS_PENDING,1);
                    uri=getContentResolver().insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI,v);
                    if(uri==null) throw new IOException("无法创建 Download 文件");
                }else{
                    File dir=Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
                    if(!dir.exists() && !dir.mkdirs()) throw new IOException("无法创建 Download 文件夹");
                ...
                if(Build.VERSION.SDK_INT>=29){
                    ...
                    if(out==null) throw new IOException("无法打开 Download 文件");
                }else{
                ...
                while((n=in.read(buf))!=-1){
                    ...
                    int pct=len>0?(int)Math.min(100,written*100/len):0;
                    double kb=written*1000.0/elapsed/1024.0;
                    final int fp=pct;
                    final double fk=kb;
                    final long fw=written;

                    runOnUiThread(()->{
                        progress.setProgress(fp);
                        progressText.setText(String.format(Locale.US,
                                "%d%% · %.1f KB/s · %s",fp,fk,formatBytes(fw)));
                    });
                }
                ...
                if(written<=0) throw new IOException("网易云返回了空文件（0 字节）");
                ...
                runOnUiThread(()->{
                    status.setText("● 下载完成："+filename);
                    progress.setProgress(100);
                    progressText.setText("100% · "+formatBytes(written));
                    Toast.makeText(this,"已保存到 Download/"+filename,Toast.LENGTH_LONG).show();
                });

            }catch(Exception e){
                // Never leave an empty/partial Download item behind after failure.
                if(uri!=null && Build.VERSION.SDK_INT>=29){
                    try{ getContentResolver().delete(uri,null,null); }catch(Exception ignored){}
                }
                final String msg=e.getMessage()==null?e.toString():e.getMessage();
                runOnUiThread(()->{
                    status.setText("● 下载失败："+msg);
                    progressText.setText("未生成有效 MP3 文件");
                });
            }finally{
